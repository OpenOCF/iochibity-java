...
int init_ResourceSPs(JNIEnv* env)
{
	...
	MID_RSP_REACT = (*env)->GetMethodID(env, K_RESOURCE_SP, "react", J_NULLARY J_VOID);
	if (MID_RSP_REACT == NULL) {
	    printf("ERROR: GetFieldID failed for 'react' of " FQCN_RESOURCE_SP "\n");
	    return -1;
	}
    }
							    ...
							    OCEntityHandlerFlag c_watch_flag)
{
    ...
    jobject j_StimulusIn = (*env)->NewObject(env, K_INBOUND_STIMULUS, MID_INBOUND_STIMULUS_CTOR); // request_in_ctor);
