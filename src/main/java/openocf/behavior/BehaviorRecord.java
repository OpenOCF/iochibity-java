...
public abstract class BehaviorRecord implements IBehaviorRecord
{
    ...
    // FIXME: uri should go in SP rather than msg?
    ...
    // setUri is only for Exhibitables (Outbound msgs)

    // network stuff? OCClientResponse.connType, OCEntityHandlerRequest.OCDevAddr.adapter/.flags

    // Payload. Every msg potentially has a payload, so it belongs here (abstractly).
