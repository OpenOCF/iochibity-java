...
//    (InboundStimulus) from the client)
...
public class OutboundResponse extends Outbound
{
    // _handle is inherited from BehaviorRecord
    // in this case, a ptr to OCEntityHandlerResponse (this is not requestHandle)

    public  InboundStimulus _requestIn;

    // CTORS
    public OutboundResponse(InboundStimulus rqst)
    {
    ...
    public OutboundResponse(InboundStimulus rqst, ObservationRecords observations)
    {
    	System.out.println("OutboundResponse CTOR 2");
    	_requestIn = rqst;
