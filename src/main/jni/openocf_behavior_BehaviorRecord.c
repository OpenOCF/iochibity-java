/**
 * @file openocf_behavior_Behavior.c
 * @author Gregg Reynolds
 * @date December 2016
 *
 * @brief Routines to convert OCPayload structs to `behavior` objects.
 */
...
#include "openocf_behavior_BehaviorRecord.h"
#include "jni_init.h"
...
#define FQCN_BEHAVIOR_RECORD "openocf/behavior/BehaviorRecord"
jclass    K_BEHAVIOR_RECORD                       = NULL;
jfieldID  FID_BEHAVIOR_HANDLE            = NULL;
...
jmethodID MID_BEHAVIOR_GET_EP          = NULL;
char     *MID_BEHAVIOR_GET_EP_NM       = "getEndpoint";
char     *MID_BEHAVIOR_GET_EP_T       = "getEndpoint";
...
{
    jclass klass;
    klass = (*env)->FindClass(env, FQCN_BEHAVIOR_RECORD);
    JNI_ASSERT_NULL(klass, ERR_MSG(ERR_CLASS, FQCN_BEHAVIOR_RECORD), -1);
    K_BEHAVIOR_RECORD = (jclass)(*env)->NewGlobalRef(env, klass);
    (*env)->DeleteLocalRef(env, klass);

    /* if (MID_MSG_GET_OBSERVATION_TYPE == NULL) { */
    /* 	MID_MSG_GET_OBSERVATION_TYPE = (*env)->GetMethodID(env, K_BEHAVIOR_RECORD, "getObservationType", "()I"); */
    /* 	if (MID_MSG_GET_OBSERVATION_TYPE == NULL) { */
    ...
    /* 	FID_MSG_OBSERVATION_HANDLE = (*env)->GetFieldID(env, K_BEHAVIOR_RECORD, "_observationRecordHandle", "J"); */
    /* 	if (FID_MSG_OBSERVATION_HANDLE == NULL) { */
    ...
    if (FID_BEHAVIOR_HANDLE == NULL) {
	FID_BEHAVIOR_HANDLE = (*env)->GetFieldID(env, K_BEHAVIOR_RECORD, "_handle", "J");
	JNI_ASSERT_NULL(FID_BEHAVIOR_HANDLE,
			ERR_MSG(ERR_FLD, "BehaviorRecord._handle"), -1);
	    }

    /* if (MID_BEHAVIOR_GET_EP == NULL) { */
    /* 	MID_BEHAVIOR_GET_EP = (*env)->GetMethodID(env, K_BEHAVIOR_RECORD, */
    /* 						  MID_BEHAVIOR_GET_EP_NM, */
    /* 						  FQCS_ENDPOINT); */
    /* 						    /\* "Lopenocf/EndPoint;"); *\/ */
    /* 	JNI_ASSERT_NULL(MID_BEHAVIOR_GET_EP, */
    /* 			ERR_MSG(ERR_FLD, "BehaviorRecord.getEndpoint"), -1); */
    /* } */

    /* if (MID_MSG_GET_REMOTE_DEVADDR == NULL) { */
    /* 	MID_MSG_GET_REMOTE_DEVADDR = (*env)->GetMethodID(env, K_BEHAVIOR_RECORD, */
    ...
    /* 	FID_MSG_METHOD = (*env)->GetFieldID(env, K_BEHAVIOR_RECORD, "_method", "I"); */
    /* 	if (FID_MSG_METHOD == NULL) { */
    ...
    /* 	MID_MSG_GET_METHOD = (*env)->GetMethodID(env, K_BEHAVIOR_RECORD, "getMethod", "()I"); */
    /* 	if (MID_MSG_GET_METHOD == NULL) { */
