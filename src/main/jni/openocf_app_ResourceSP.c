...
int init_ResourceSPs(JNIEnv* env)
{
	...
	MID_RSP_REACT = (*env)->GetMethodID(env, K_RESOURCE_SP,
					    "react",
					    "(Lopenocf/behavior/InboundStimulus;)V");
	if (MID_RSP_REACT == NULL) {
...
/**  OBSOLETE
 * @brief Convert incoming `OCEntityHandlerRequest` to `InboundStimulus`
 * object.  Internal implementation routine called by
 * `c_resource_sp_react`.  The resulting InboundStimulus contains only
...
/* THROW_JNI_EXCEPTION won't work since we're not called from Java */
 jobject c_resource_sp_OCEntityHandlerRequest_to_StimulusIn(JNIEnv* env,
							    OCEntityHandlerRequest* c_EHRequest,
							    OCEntityHandlerFlag c_watch_flag)
{
    printf("%s: %s ENTRY\n", __FILE__, __func__);

    jobject j_StimulusIn = (*env)->NewObject(env, K_INBOUND_STIMULUS, MID_INBOUND_STIMULUS_CTOR);

    (*env)->SetLongField(env, j_StimulusIn,
			 FID_BEHAVIOR_HANDLE, (intptr_t)c_EHRequest);
   ...
   /* (*env)->SetLongField(env, j_StimulusIn, */
    ...
    /* (*env)->SetObjectField(env, j_StimulusIn, FID_INBOUND_STIMULUS_QUERY, js); */
    ...
    /* 	    /\* c_EHRequest->obsInfo.action *\/ */
    /* 	    /\* (*env)->SetIntField(env, j_StimulusIn, FID_INBOUND_STIMULUS_WATCH_ACTION, c_EHRequest->obsInfo.action); *\/ */
    /* 	    /\* /\\* c_EHRequest->obsInfo.Id *\\/ *\/ */
    /* 	    /\* (*env)->SetIntField(env, j_StimulusIn, FID_INBOUND_STIMULUS_WATCH_ID, c_EHRequest->obsInfo.obsId); *\/ */
    /* 	} else { */
    ...
    /* message ID */
    /* (*env)->SetIntField(env, j_StimulusIn, FID_INBOUND_STIMULUS_MSG_ID, c_EHRequest->messageID); */
